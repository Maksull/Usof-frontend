...
const SingleComment: React.FC<SingleCommentProps> = ({
    ...
    return (
        <div className="w-full mb-4">
            <div id={`comment-${comment.id}`} className={`relative flex flex-col ${depth > 0 ? 'ml-4 md:ml-8 pl-4 border-l-2 border-gray-200 dark:border-gray-700' : ''}`}>
                {replyToComment && (
                    <button onClick={() => {
                        ...
                        <span>Replying to @{replyToComment.author?.login ?? "unknown"}</span>
                    </button>
                )}
                <div className="flex items-start gap-4">
                    <div className="flex-shrink-0">
                        {comment.author?.profilePicture ? (
                            <img src={comment.author.profilePicture} alt={comment.author.login || "User"}
                    ...
                    <div className="flex-grow overflow-hidden">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <button onClick={() => onCommentLike(comment.id, true)} disabled={isLiking}
                            ...
                            <button onClick={() => onCommentLike(comment.id, false)} disabled={isLiking}
                                className={`flex items-center text-xs md:text-sm px-2 py-1 rounded-md transition-all 
                                    ${isLiking ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}
            ...
            {showReplies && replies.map(reply => (
                <SingleComment key={reply.id} comment={reply} allComments={allComments} setPost={setPost}
                    isLiking={isLiking} onCommentLike={onCommentLike} depth={depth + 1} />
            ))}
        </div>
    );
};
